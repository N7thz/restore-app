...
export async function updateProductEntry(
	...
	const { productId, createdAt } = await prisma.productEntry.update({
		where: { id },
		data,
	})
	...
	const totalEntries = productEntries.reduce(
		(sum, entry) => sum + entry.quantity,
		0
	)

	const totalExits = productExits.reduce((sum, exit) => sum + exit.quantity, 0)
